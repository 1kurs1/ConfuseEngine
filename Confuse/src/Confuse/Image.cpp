...
namespace Confuse {
	...
	namespace Utils {
		...
		static VkFormat confuseFormatToVulkanFormat(ImageFormat format){
			switch (format){
	...
	Image::Image(uint32_t width, uint32_t height, ImageFormat format, const void* data) : m_width(width), m_height(height), m_format(format){
		allocateMemory(m_width * m_height * Utils::bytesPerPixel(m_format));
		if (data)
			setData(data);
	}
	...
	void Image::allocateMemory(uint64_t size){
		...
		VkFormat vulkanFormat = Utils::confuseFormatToVulkanFormat(m_format);
	...
	void Image::release(){
		Application::submitResourceFree([sampler = m_sampler, imageView = m_imageView, image = m_image, memory = m_memory, stagingBuffer = m_stagingBuffer, stagingBufferMemory = m_stagingBufferMemory](){
			VkDevice device = Application::getDevice();
