...
namespace ConfuseEngineRenderer {
    ...
    CE_SwapChain::~CE_SwapChain() {
        ...
        for (int i = 0; i < m_depthImages.size(); i++) {
            vkDestroyImageView(m_rDevice.device(), m_depthImageViews[i], nullptr);
            vkDestroyImage(m_rDevice.device(), m_depthImages[i], nullptr);
            vkFreeMemory(m_rDevice.device(), m_depthImageMemorys[i], nullptr);
        }
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            vkDestroySemaphore(m_rDevice.device(), m_renderFinishedSemaphores[i], nullptr);
            vkDestroySemaphore(m_rDevice.device(), m_imageAvailableSemaphores[i], nullptr);
            vkDestroyFence(m_rDevice.device(), m_inFlightFences[i], nullptr);
        }
    }
    ...
    VkResult CE_SwapChain::submitCommandBuffers(const VkCommandBuffer *buffers, uint32_t *imageIndex) {
        if (m_imagesInFlight[*imageIndex] != VK_NULL_HANDLE) {
            vkWaitForFences(m_rDevice.device(), 1, &m_imagesInFlight[*imageIndex], VK_TRUE, UINT64_MAX);
        }
    ...
    void CE_SwapChain::createSwapChain() {
        ...
        createInfo.surface = m_rDevice.surface();
    ...
    void CE_SwapChain::createRenderPass() {
    ...
    void CE_SwapChain::createFramebuffers() {
        ...
        for (size_t i = 0; i < imageCount(); i++) {
            ...
            if (vkCreateFramebuffer(m_rDevice.device(), &framebufferInfo, nullptr, &m_swapChainFramebuffers[i]) != VK_SUCCESS) {
                throw std::runtime_error("failed to create framebuffer!");
            }
        }
    }

    void CE_SwapChain::createDepthResources() {
        ...
        for (int i = 0; i < m_depthImages.size(); i++) {
            ...
            m_rDevice.createImageWithInfo(imageInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_depthImages[i], m_depthImageMemorys[i]);
            ...
            if (vkCreateImageView(m_rDevice.device(), &viewInfo, nullptr, &m_depthImageViews[i]) != VK_SUCCESS) {
                throw std::runtime_error("failed to create texture image view!");
            }
        }
    }

    void CE_SwapChain::createSyncObjects() {
    ...
    VkFormat CE_SwapChain::findDepthFormat() {
